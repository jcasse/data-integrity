/* Copyright (c) 2016 Juan Casse. All rights reserved.
...
void parse_command_line_arguments(struct param * p, int argc, char *argv[])
{
	...
	/* Get options. */
	while ((c = getopt(argc, argv, "hs:mrwxb:e:i:z:")) != -1) {
		switch (c) {
		case 'h':
			print_help();
			exit(0);
		case 's':
			...
			if (seed == 0ul)
				abort("Error reading -s argument\n");
			if (seed == ULONG_MAX)
				abort("Error -s value out of range\n");
			p->seed = seed;
			break;
		case 'm':
...
int chunk_cmp(const struct data *a, const struct data *b)
{
	int count = 0;
	if (a->lba != b->lba)
		count++;
...
void write_chunk(struct stats *s, struct data *chunk, struct param *p)
{
	/*
...
void read_chunk(struct stats *s, struct data *chunk, struct param *p)
{
	/*
	 * There are a number of restrictions with direct I/O (O_DIRECT):
	 * -Data buffer must be aligned on memory boundary,
...
void execute(struct stats *s, struct param *p)
{
	/* If read-only workload, initial pass to recreate LBAs then verify. */
	uint64_t iterations = p->itr;
	if (p->wrk == r_only)
		iterations = p->itr + 1;

	/* Perform read/write/verify. */
	uint64_t i;
	for (i = 0; i < iterations; i++) {
		...
		while ((*p->wrk)(&lba, &op, i, p->seq, p)) {
			...
			*/
			...
			/*
		 	* Verify:
		 	* Iterations simulate writes to compute the generations,
		 	* but not really write the blocks.
